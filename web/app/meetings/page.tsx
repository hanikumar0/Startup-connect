...
export default function MeetingsPage() {
    ...
    useEffect(() => {
        async function fetchMeetings() {
            try {
                const data = await apiFetchJSON("/api/meetings/my-meetings");
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case "UPCOMING": return <Badge className="bg-slate-900 text-white border-none text-[8px] font-black uppercase tracking-widest px-2 italic">Scheduled</Badge>;
            case "LIVE": return <Badge className="bg-emerald-500 text-white border-none text-[8px] font-black uppercase tracking-widest px-2 animate-pulse italic">Active Node</Badge>;
            default: return <Badge variant="outline" className="text-[8px] font-black uppercase tracking-widest px-2 opacity-50">Archive</Badge>;
        }
    };
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="relative hidden sm:block">
                            ...
                            <Input placeholder="SEARCH SESSIONS..." className="w-64 pl-10 h-10 border-slate-200 bg-white text-[10px] font-black uppercase tracking-widest focus-visible:ring-slate-200" />
                        </div>
                        <Button className="h-10 bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest px-6 italic shadow-lg shadow-slate-200">
                                 ...
                                 {meetings.filter(m => m.status === 'UPCOMING').slice(0, 3).map((m, i) => (
                                    <div key={i} className="flex gap-3">
                                       <div className="h-10 w-10 shrink-0 rounded-lg bg-slate-50 border border-slate-100 flex flex-col items-center justify-center text-slate-900">
                                          ...
                                          <span className="text-[7px] font-black uppercase tracking-tighter opacity-40">MTH</span>
                                       </div>
                                       <div className="min-w-0">
                                          <p className="text-[10px] font-black text-slate-900 uppercase truncate italic">{m.title}</p>
                                          <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">{new Date(m.startTime).getHours()}:00</p>
                                       </div>
                                    </div>
                                 ))}
                              </div>
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        {loading ? (
                                        ...
                                        <CardContent className="p-0">
                                            <div className="flex flex-col md:flex-row md:items-center">
                                                ...
                                                <div className="p-8 border-t md:border-t-0 md:border-l border-slate-100 flex items-center gap-3 bg-slate-50/30">
                                                    <Button variant="outline" className="h-12 w-12 rounded-xl border-slate-200 text-slate-300 hover:text-slate-900" onClick={() => router.push(`/meetings/room/${meeting.roomId}`)}>
                                                       <MoreVertical size={20} />
                                                    </Button>
                                                    <Button className="h-12 bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest px-8 italic shadow-lg shadow-slate-200" onClick={() => router.push(`/meetings/${meeting.roomId}`)}>
                                                       Join Intel <ExternalLink size={14} className="ml-2" />
                                                    </Button>
                                                </div>
                                            </div>
                                        </CardContent>
                          ...
                          <div className="flex flex-col items-center justify-center p-20 bg-slate-50/50 rounded-2xl border-2 border-dashed border-slate-100 text-center space-y-6">
                             ...
                             <Button variant="outline" className="h-10 border-slate-200 text-[10px] font-black uppercase tracking-widest px-8 italic" onClick={() => router.push("/discover")}>
                                Synchronize Now
                             </Button>
                          </div>
                        )}
