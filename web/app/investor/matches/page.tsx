        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-border pb-10">
          <div className="space-y-1">
            ...
            <p className="text-sm text-slate-500 font-medium">
               Analyzing <span className="text-slate-900 font-bold">{matches.length} companies</span> that align with your investment thesis.
            </p>
          </div>
        ...
        {isLoading ? (
            <div className="flex h-[40vh] items-center justify-center">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-xs font-bold tracking-widest uppercase text-slate-400">Scanning deal-flow...</p>
                </div>
            </div>
        ) : matches.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {matches.map((match, idx) => (
                    <MatchCard key={idx} match={match} type="startup" />
                ))}
            </div>
        ) : (
            <div className="py-24 text-center bg-white border border-border border-dashed rounded-lg">
                ...
                <p className="text-slate-500 mt-2 max-w-sm mx-auto text-sm">
                    We haven't identified new companies in your target sector this morning. Try adjusting your ticket size or location filters.
                </p>
                <Link href="/settings">
